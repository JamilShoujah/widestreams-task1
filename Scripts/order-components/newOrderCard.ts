...
export const NewOrderCard = (
  ...
  card.querySelector('.save-order-btn')!.addEventListener('click', () => {
    const item = (card.querySelector('.input-item') as HTMLInputElement).value;
    const quantity = Number((card.querySelector('.input-quantity') as HTMLInputElement).value);
    const orderDate = (card.querySelector('.input-order-date') as HTMLInputElement).value;
    const flightId = Number((card.querySelector('.select-flight') as HTMLSelectElement).value);
    const orderedBy = (card.querySelector('.select-ordered-by') as HTMLSelectElement).value;

    if (!orderedBy) return alert('Please select who ordered this');

    const selectedFlight = flights.find((f) => f.id === flightId);
    if (!selectedFlight) return alert('Please select a valid flight');
